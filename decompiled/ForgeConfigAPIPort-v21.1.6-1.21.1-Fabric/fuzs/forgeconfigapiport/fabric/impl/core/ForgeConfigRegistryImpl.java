...
public final class ForgeConfigRegistryImpl implements ForgeConfigRegistry {
   ...
   public ModConfig register(String modId, ModConfig.Type type, IConfigSpec<?> spec) {
      ModConfig modConfig = ConfigTracker.INSTANCE.registerConfig(unwrap(type), new ForgeConfigSpecAdapter(spec), modId).modConfig;
   ...
   public ModConfig register(String modId, ModConfig.Type type, IConfigSpec<?> spec, String fileName) {
      ModConfig modConfig = ConfigTracker.INSTANCE.registerConfig(unwrap(type), new ForgeConfigSpecAdapter(spec), modId, fileName).modConfig;
      Objects.requireNonNull(modConfig, "mod config is null");
      return modConfig;
   }
   ...
   public static ModConfig adapt(net.neoforged.fml.config.ModConfig modConfig) {
      return modConfig.getSpec() instanceof ForgeConfigSpecAdapter adapter
         ? new ModConfig(wrap(modConfig.getType()), adapter.spec(), modConfig.getModId(), modConfig.getFileName(), modConfig)
         : null;
   }
}
