package com.natamus.youritemsaresafe_common_forge.util;
...
public class Util {
   ...
   public static void failureMessage(Player player, int planksLeft, int stoneLeft, int planksNeeded, int stoneNeeded) {
      ...
      if (planksLeft != planksNeeded) {
         ItemEntity planks = new ItemEntity(level, vec.x, vec.y + 1.0, vec.z, new ItemStack(Items.OAK_PLANKS, planksNeeded - planksLeft));
         level.addFreshEntity(planks);
      }

      if (stoneLeft != stoneNeeded) {
         ItemEntity stones = new ItemEntity(level, vec.x, vec.y + 1.0, vec.z, new ItemStack(Items.STONE, stoneNeeded - stoneLeft));
