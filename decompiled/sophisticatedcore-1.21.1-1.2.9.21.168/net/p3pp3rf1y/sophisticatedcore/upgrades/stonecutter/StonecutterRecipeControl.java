...
public class StonecutterRecipeControl extends WidgetBase {
   ...
   private void drawRecipesItems(class_332 guiGraphics, int listInnerLeftX, int top, int recipeIndexOffsetMax) {
      List<class_8786<class_3975>> list = this.container.getRecipeList();
      ...
      for (int i = this.recipeIndexOffset; i < recipeIndexOffsetMax && i < this.container.getRecipeList().size(); i++) {
         ...
         int l = j / 4;
         int i1 = top + l * 18 + 2;
         GuiHelper.renderItemInGUI(
            guiGraphics, this.minecraft, ((class_3975)list.get(i).comp_1933()).method_8110(this.minecraft.field_1687.method_30349()), k, i1
         );
      }
   }
      ...
      if (this.hasItemsInInputSlot) {
         ...
         for (int recipeIndex = this.recipeIndexOffset; recipeIndex < k && recipeIndex < list.size(); recipeIndex++) {
            ...
            if (mouseX >= recipeLeftX && mouseX < recipeLeftX + 16 && mouseY >= k1 && mouseY < k1 + 18) {
               this.renderTooltip(
                  guiGraphics, ((class_3975)list.get(recipeIndex).comp_1933()).method_8110(this.minecraft.field_1687.method_30349()), mouseX, mouseY
               );
            }
         }
      }
   ...
   public boolean method_25402(double mouseX, double mouseY, int button) {
      ...
      if (this.hasItemsInInputSlot) {
         ...
         for (int recipeIndex = this.recipeIndexOffset; recipeIndex < maxRecipeIndexOffset; recipeIndex++) {
            ...
            if (relativeX >= 0.0 && relativeY >= 0.0 && relativeX < 16.0 && relativeY < 18.0 && this.container.selectRecipe(recipeIndex)) {
