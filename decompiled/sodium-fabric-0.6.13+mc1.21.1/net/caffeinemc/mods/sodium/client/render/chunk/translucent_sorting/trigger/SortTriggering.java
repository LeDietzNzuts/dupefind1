...
public class SortTriggering {
   ...
   void triggerSectionDirect(class_4076 sectionPos) {
      if (this.isCatchingUp()) {
         this.triggerSectionCatchup(sectionPos.method_18694(), true);
      } else {
         this.triggerSectionCallback.accept(sectionPos.method_18694(), true);
   ...
   public void applyTriggerChanges(DynamicTopoData data, DynamicTopoData.DynamicTopoSorter topoSorter, class_4076 pos, Vector3dc cameraPos) {
      if (data.isMatchingSorter(topoSorter)) {
         if (data.checkAndApplyGFNITriggerOff(topoSorter)) {
            this.gfni.removeSection(pos.method_18694(), data);
         }
         ...
         if (data.checkAndApplyDirectTriggerOff(topoSorter)) {
            this.direct.removeSection(pos.method_18694(), data);
         }
   ...
   public void integrateTranslucentData(TranslucentData oldData, TranslucentData newData, Vector3dc cameraPos, BiConsumer<Long, Boolean> triggerSectionCallback) {
      if (oldData != newData) {
         class_4076 pos = newData.sectionPos;
         this.incrementSortTypeCounter(newData);
         if (newData instanceof DynamicData dynamicData) {
            this.direct.removeSection(pos.method_18694(), oldData);
            this.decrementSortTypeCounter(oldData);
            this.triggerSectionCallback = triggerSectionCallback;
            ...
            this.removeSection(oldData, pos.method_18694());
         }
      }
   }
   ...
   interface SectionTriggers<T extends DynamicData> {
      ...
      void integrateSection(SortTriggering var1, class_4076 var2, T var3, CameraMovement var4);
   }
}
