...
public class VertexSerializerFactory {
   ...
   public static VertexSerializerFactory.Bytecode generate(class_293 srcFormat, class_293 dstFormat, String identifier) {
      ...
      methodVisitorx.visitVarInsn(22, 1);
      methodVisitorx.visitLdcInsn((long)srcFormat.method_1362());
      methodVisitorx.visitInsn(97);
      ...
      methodVisitorx.visitVarInsn(22, 3);
      methodVisitorx.visitLdcInsn((long)dstFormat.method_1362());
      methodVisitorx.visitInsn(97);
   ...
   private static List<VertexSerializerFactory.MemoryTransfer> createMemoryTransferList(class_293 srcVertexFormat, class_293 dstVertexFormat) {
      ArrayList<VertexSerializerFactory.MemoryTransfer> ops = new ArrayList<>();

      for (int elementIndex = 0; elementIndex < 32; elementIndex++) {
         class_296 elementType = class_296.method_60844(elementIndex);
         if (elementType != null && dstVertexFormat.method_60836(elementType)) {
            if (!srcVertexFormat.method_60836(elementType)) {
               throw new RuntimeException("Source format is missing element %s as required by destination format".formatted(elementType));
            }

            int srcOffset = srcVertexFormat.method_60835(elementType);
            int dstOffset = dstVertexFormat.method_60835(elementType);
            ops.add(new VertexSerializerFactory.MemoryTransfer(srcOffset, dstOffset, elementType.method_60847()));
         }
      }
