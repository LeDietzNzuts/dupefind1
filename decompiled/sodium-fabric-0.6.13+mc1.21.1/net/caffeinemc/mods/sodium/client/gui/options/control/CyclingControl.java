...
public class CyclingControl<T extends Enum<T>> implements Control<T> {
   ...
   public CyclingControl(Option<T> option, Class<T> enumType, class_2561[] names) {
   ...
   public CyclingControl(Option<T> option, Class<T> enumType, T[] allowedValues) {
      ...
      this.names = new class_2561[universe.length];
      ...
      for (int i = 0; i < this.names.length; i++) {
         ...
         if (value instanceof TextProvider) {
            ...
            name = class_2561.method_43470(value.name());
         }
   ...
   private static class CyclingControlElement<T extends Enum<T>> extends ControlElement<T> {
      ...
      public CyclingControlElement(Option<T> option, Dim2i dim, T[] allowedValues, class_2561[] names) {
         super(option, dim);
         this.allowedValues = allowedValues;
         this.names = names;
         this.currentIndex = 0;
      ...
      public void method_25394(class_332 graphics, int mouseX, int mouseY, float delta) {
         ...
         class_2561 name = this.names[value.ordinal()];
         int strWidth = this.getStringWidth(name);
         this.drawString(graphics, name, this.dim.getLimitX() - strWidth - 6, this.dim.getCenterY() - 4, -1);
      }

      public boolean method_25402(double mouseX, double mouseY, int button) {
         if (this.option.isAvailable() && button == 0 && this.dim.containsCursor(mouseX, mouseY)) {
            this.cycleControl(class_437.method_25442());
            this.playClickSound();
            return true;
         } else {
            return false;
