...
public final class GuiRegistry implements GuiRegistryAccess {
   ...
   private static <T> Optional<T> firstPresent(Stream<Supplier<Optional<T>>> optionals) {
      return (Optional<T>)optionals.map(Supplier::get).filter(Optional::isPresent).findFirst().orElse((T)Optional.empty());
   }

   @Override
   public List<AbstractConfigListEntry> get(String i18n, Field field, Object config, Object defaults, GuiRegistryAccess registry) {
      return firstPresent(
            Arrays.stream(GuiRegistry.Priority.values())
               .map(priority -> () -> this.providers.get(priority).stream().filter(entry -> entry.predicate.test(field)).findFirst())
         )
   ...
   public List<AbstractConfigListEntry> transform(
      ...
      for (GuiTransformer transformer : this.transformers
         .stream()
   ...
   private void registerProvider(GuiRegistry.Priority priority, GuiProvider provider, Predicate<Field> predicate) {
      this.providers.computeIfAbsent(priority, p -> new ArrayList<>()).add(new GuiRegistry.ProviderEntry(predicate, provider));
   }
