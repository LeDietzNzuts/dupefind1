...
public abstract class ScrollingContainer {
   ...
   public final int scrollAmountInt() {
      return (int)Math.round(this.scroll.value());
   }
   ...
   public final void offset(double value, boolean animated) {
      this.scrollTo(this.scroll.target() + value, animated);
   }
   ...
   public void updatePosition(float delta) {
      this.scroll.setTarget(handleBounceBack(this.scrollTarget(), this.getMaxScroll(), delta));
   ...
   public static double handleBounceBack(double target, double maxScroll, float delta, double bounceBackMultiplier) {
      if (bounceBackMultiplier >= 0.0) {
         ...
         if (target < 0.0) {
            target -= target * (1.0 - bounceBackMultiplier) * delta / 3.0;
         } else if (target > maxScroll) {
            target = (target - maxScroll) * (1.0 - (1.0 - bounceBackMultiplier) * delta / 3.0) + maxScroll;
         }
      } else {
      ...
      if (this.hasScrollBar()) {
         ...
         height = (int)(
            height
               - Math.min(
                  (double)(
                     this.scrollAmount() < 0.0 ? (int)(-this.scrollAmount()) : (this.scrollAmount() > maxScroll ? (int)this.scrollAmount() - maxScroll : 0)
                  ),
                  height * 0.95
               )
         );
