   ...
   public boolean shouldApplyMixin(String targetClassName, String mixinClassName) {
      if (!mixinClassName.startsWith(this.mixinPackage)) {
         ...
         if (!ImmediatelyFast.config.font_atlas_resizing && packageName.startsWith("font_atlas_resizing")) {
            ...
            return false;
         } else if (packageName.startsWith("core.compat.iris") && PlatformCode.getModVersion("iris").isEmpty()) {
            return false;
         } else if (packageName.startsWith("hud_batching.compat.appleskin") && PlatformCode.getModVersion("appleskin").isEmpty()) {
            return false;
         } else if (packageName.startsWith("hud_batching.compat.journeymap") && PlatformCode.getModVersion("journeymap").isEmpty()) {
            return false;
         } else {
            return packageName.startsWith("hud_batching.compat.xaerominimap") && PlatformCode.getModVersion("xaerominimap").isEmpty()
               ? false
               : !packageName.startsWith("hud_batching.compat.ftbchunks") || !PlatformCode.getModVersion("ftbchunks").isEmpty();
         }
      }
   }
