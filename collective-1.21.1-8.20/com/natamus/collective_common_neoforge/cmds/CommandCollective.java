package com.natamus.collective_common_neoforge.cmds;
...
public class CommandCollective {
   public static void register(CommandDispatcher<CommandSourceStack> dispatcher) {
      dispatcher.register(
         (LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)((LiteralArgumentBuilder)Commands.literal("collective")
                     .requires(commandSourceStack -> commandSourceStack.isPlayer()))
                  .executes(command -> showCommandHelp((CommandSourceStack)command.getSource())))
               .then(Commands.literal("help").executes(command -> showCommandHelp((CommandSourceStack)command.getSource()))))
            .then(Commands.literal("reset").then(Commands.literal("headcache").executes(command -> {
   ...
   private static int showCommandHelp(CommandSourceStack source) {
      if (source.hasPermission(2)) {
         MessageFunctions.sendMessage(source, Component.literal("Collective Admin Usage:").withStyle(ChatFormatting.GOLD), true);
