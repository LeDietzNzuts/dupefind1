...
public class NeoForgeTeleportHelper implements TeleportHelper {
   ...
   public boolean teleportEntity(Entity entity, ServerLevel serverLevel, BlockPos blockPos) {
      return entity.teleportTo(
         serverLevel, blockPos.getX() + 0.5, blockPos.getY(), blockPos.getZ() + 0.5, RelativeMovement.ALL, entity.getYRot(), entity.getXRot()
      );
   }

   @Override
   public boolean teleportEntity(Entity entity, ResourceKey<Level> targetDimension, Vec3 vec3) {
      return entity.level().isClientSide ? false : this.teleportEntity(entity, entity.getServer().getLevel(targetDimension), vec3);
   }

   @Override
   public boolean teleportEntity(Entity entity, ResourceKey<Level> targetDimension, BlockPos blockPos) {
      return entity.level().isClientSide ? false : this.teleportEntity(entity, entity.getServer().getLevel(targetDimension), blockPos);
   }
}
