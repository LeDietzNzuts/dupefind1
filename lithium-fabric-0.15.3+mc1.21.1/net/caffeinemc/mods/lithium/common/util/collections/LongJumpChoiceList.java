...
public class LongJumpChoiceList extends AbstractList<class_6031> {
            ...
            for (int y = -verticalRange; y <= verticalRange; y++) {
               for (int z = -horizontalRange; z <= horizontalRange; z++) {
                  ...
                  if (index >= 0) {
                     ...
                     this.weightByDistanceSq[index] = this.weightByDistanceSq[index] + squaredDistance;
   ...
   public LongJumpChoiceList(class_2338 origin, IntArrayList[] packedOffsetsByDistanceSq, int[] weightByDistanceSq, int totalWeight) {
      this.origin = origin;
   ...
   public static LongJumpChoiceList forCenter(class_2338 centerPos, byte horizontalRange, byte verticalRange) {
      if (horizontalRange >= 0 && verticalRange >= 0) {
         short range = (short)(horizontalRange << 8 | verticalRange);
         LongJumpChoiceList jumpDestinationsList;
         if (range == 1026) {
            jumpDestinationsList = FROG_JUMP;
         } else if (range == 1285) {
            jumpDestinationsList = GOAT_JUMP;
         } else {
            jumpDestinationsList = CHOICE_LISTS.computeIfAbsent(
               ByteBytePair.of(horizontalRange, verticalRange), key -> new LongJumpChoiceList(key.leftByte(), key.rightByte())
            );
         }
   ...
   private LongJumpChoiceList offsetCopy(class_2338 offset) {
      ...
      return new LongJumpChoiceList(
         this.origin.method_10081(offset), packedOffsetsByDistanceSq, Arrays.copyOf(this.weightByDistanceSq, this.weightByDistanceSq.length), this.totalWeight
      );
   }

   public class_6031 removeRandomWeightedByDistanceSq(class_5819 random) {
      int targetWeight = random.method_43048(this.totalWeight);
      ...
      for (int index = 0; targetWeight >= 0 && index < this.weightByDistanceSq.length; index++) {
         ...
         if (targetWeight < 0) {
            ...
            int elementIndex = random.method_43048(elementsOfDistance.size());
            ...
            this.weightByDistanceSq[index] = this.weightByDistanceSq[index] - distanceSq;
            this.totalWeight -= distanceSq;
            return new class_6031(this.origin.method_10069(this.unpackX(packedOffset), this.unpackY(packedOffset), this.unpackZ(packedOffset)), distanceSq);
         }
      }
   ...
   public class_6031 get(int index) {
   ...
   public class_6031 remove(int index) {
      int elementIndex = index;
      IntArrayList[] offsetsByDistanceSq = this.packedOffsetsByDistanceSq;
      ...
      for (int distanceSq = 0; distanceSq < offsetsByDistanceSq.length; distanceSq++) {
         ...
         if (packedOffsets != null) {
            if (elementIndex < packedOffsets.size()) {
               ...
               this.weightByDistanceSq[distanceSq] = this.weightByDistanceSq[distanceSq] - distanceSq;
               this.totalWeight -= distanceSq;
               return new class_6031(this.origin.method_10069(this.unpackX(packedOffset), this.unpackY(packedOffset), this.unpackZ(packedOffset)), distanceSq);
            }
